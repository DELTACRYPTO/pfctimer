...
contract DuelEliminatoire {
    ...
    uint256 public inscriptionDebut;
    uint256 public delaiInscription = 3 minutes;  // Délai de 3 minutes pour inscrire le second joueur
    ...
    event TempsEcoule(string message);
    event InscriptionAnnulee(string message);
    ...
    function inscrireTrump() public {
        ...
        joueur1 = msg.sender;
        inscriptionDebut = block.timestamp;  // Démarrer le timer de 3 minutes
    ...
    function inscrirePoutine() public {
        ...
        require(block.timestamp <= inscriptionDebut + delaiInscription, "Temps d'inscription expire.");

        joueur2 = msg.sender;
        emit ChoixAttendu(joueur2, "Vous etes POUTINE. Faites votre choix: pierre, feuille ou ciseaux.");
    }

    // Vérifier si le temps d'inscription est expiré
    function verifierInscription() public {
        if (joueur1 != address(0) && joueur2 == address(0) && block.timestamp > inscriptionDebut + delaiInscription) {
            // Si le deuxième joueur ne s'est pas inscrit dans les 3 minutes
            joueur1 = address(0);  // Annuler l'inscription de TRUMP
            emit InscriptionAnnulee("Aucun joueur n'a rejoint a temps. L'inscription est annulee.");
        }
    }
